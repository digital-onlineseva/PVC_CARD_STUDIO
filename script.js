...
function attachEventListeners() {
    ...
    canvasContainer.addEventListener('wheel', (e) => {
        ...
        if (state.viewScale < 0.2) state.viewScale = 0.2;
        if (state.viewScale > 5.0) state.viewScale = 5.0;
        applyViewScale();
    }, { passive: false });
    ...
    DOM.zoomSlider.addEventListener('input', (e) => {
        ...
        DOM.zoomValueInput.value = e.target.value;
        render();
    });
    DOM.zoomSlider.addEventListener('change', saveState);

    DOM.zoomValueInput.addEventListener('input', (e) => {
        if(!state.image) return;
